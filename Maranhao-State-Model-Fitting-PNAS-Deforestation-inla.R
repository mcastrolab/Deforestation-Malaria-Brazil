...
d1_full <- d1_full[d1_full$uf_exp==21]
...
muns_unique$munID_correct <- 1:217
...
m21_MA_results <- inla(m21_MA, family="zeroinflatedpoisson0", data=d1_full,
                       # control.predictor=list(compute=TRUE,link=NA),
                       control.compute=list(dic=TRUE, waic = TRUE, cpo = TRUE, config=TRUE),
                       # control.inla=list(tolerance=0.01),
                       verbose = T, safe = T)

m21_MA_summary <- exp(m21_MA_results$summary.fixed)

write.csv(m21_MA_summary,file="MA_model_output.csv")
