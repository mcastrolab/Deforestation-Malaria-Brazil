...
d1_full <- d1_full[d1_full$uf_exp==51]
...
muns_unique$munID_correct <- 1:141
...
m21_MT_results <- inla(m21_MT, family="zeroinflatedpoisson0", data=d1_full,
                       # control.predictor=list(compute=TRUE,link=NA),
                       control.compute=list(dic=TRUE, waic = TRUE, cpo = TRUE, config=TRUE),
                       # control.inla=list(tolerance=0.01),
                       verbose = T, safe = T)

m21_MT_summary <- exp(m21_MT_results$summary.fixed)

write.csv(m21_MT_summary,file="MT_model_output.csv")
