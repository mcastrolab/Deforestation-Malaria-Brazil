...
d1_full <- d1_full[d1_full$uf_exp==14]
...
muns_unique$munID_correct <- 1:15
...
m21_RR_results <- inla(m21_RR, family="zeroinflatedpoisson0", data=d1_full[d1_full$uf_exp==14],
                       # control.predictor=list(compute=TRUE,link=NA),
                       control.compute=list(dic=TRUE, waic = TRUE, cpo = TRUE, config=TRUE),
                       # control.inla=list(tolerance=0.01),
                       verbose = T, safe = T)

m21_RR_summary <- exp(m21_RR_results$summary.fixed)



write.csv(m21_RR_summary,file="RR_model_output.csv")
