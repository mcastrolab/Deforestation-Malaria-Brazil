...
d1_full <- d1_full[d1_full$uf_exp==13]
...
muns_unique$munID_correct <- 1:62
...
m21_AM_results <- inla(m21_AM, family="zeroinflatedpoisson0", data=d1_full,
                       # control.predictor=list(compute=TRUE,link=NA),
                       control.compute=list(dic=TRUE, waic = TRUE, cpo = TRUE, config=TRUE),
                       # control.inla=list(tolerance=0.01),
                       verbose = T, safe = T)

m21_AM_summary <- exp(m21_AM_results$summary.fixed)

write.csv(m21_AM_summary,file="AM_model_output.csv")
