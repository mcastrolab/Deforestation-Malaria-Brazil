...
d1_full <- d1_full[d1_full$uf_exp==15]
...
muns_unique$munID_correct <- 1:143
...
m21_PA_results <- inla(m21_PA, family="zeroinflatedpoisson0", data=d1_full,
                       # control.predictor=list(compute=TRUE,link=NA),
                       control.compute=list(dic=TRUE, waic = TRUE, cpo = TRUE, config=TRUE),
                       # control.inla=list(tolerance=0.01),
                       verbose = T, safe = T)

m21_PA_summary <- exp(m21_PA_results$summary.fixed)

write.csv(m21_PA_summary,file="PA_model_output.csv")
