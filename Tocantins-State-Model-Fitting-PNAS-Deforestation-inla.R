...
d1_full <- d1_full[d1_full$uf_exp==17]
...
muns_unique$munID_correct <- 1:139
...
m21_TO_results <- inla(m21_TO, family="zeroinflatedpoisson0", data=d1_full,
                       # control.predictor=list(compute=TRUE,link=NA),
                       control.compute=list(dic=TRUE, waic = TRUE, cpo = TRUE, config=TRUE),
                       # control.inla=list(tolerance=0.01),
                       verbose = T, safe = T)

m21_TO_summary <- exp(m21_TO_results$summary.fixed)


write.csv(m21_TO_summary,file="TO_model_output.csv")
